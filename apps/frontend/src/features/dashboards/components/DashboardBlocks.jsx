...
export const DashboardHero = ({ eyebrow, title, description, metrics = [] }) => (
  <Card className="overflow-hidden border-white/10 bg-[radial-gradient(circle_at_top_left,rgba(245,158,11,0.16),transparent_28%),linear-gradient(135deg,rgba(255,255,255,0.08),rgba(255,255,255,0.02))] p-0">
    <div className="relative overflow-hidden p-6 lg:p-8">
      ...
      <div className="relative space-y-6">
        <div className="space-y-3">
          <p className="text-xs font-semibold uppercase tracking-[0.3em] text-amber-100/80">{eyebrow}</p>
        ...
        {metrics.length ? (
          <div className="flex flex-wrap gap-3">
            {metrics.map((metric) => (
              <div key={metric.label} className="rounded-full border border-white/10 bg-black/20 px-4 py-2 text-sm text-stone-100 backdrop-blur">
  ...
  <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
    {items.map((item) => (
      <Card key={item.label} className="border-white/10 bg-white/[0.04]">
        <div className="space-y-3">
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-stone-500">{item.label}</p>
          <p className="text-3xl font-semibold text-white">{item.value}</p>
          <p className="text-sm leading-6 text-stone-300">{item.helpText}</p>
        </div>
      </Card>
    ))}
  </div>
  ...
  <Card className={cn("border-white/10 bg-white/[0.04]", className)}>
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <div className="space-y-2">
        ...
        {description ? <p className="text-sm leading-6 text-stone-300">{description}</p> : null}
      </div>
...
export const DashboardEmptyState = ({ title, description, actionLabel, onAction }) => (
  <div className="rounded-[1.75rem] border border-dashed border-white/15 bg-stone-950/50 px-5 py-8 text-center">
...
export const DashboardLoadingState = ({ label = "Chargement..." }) => (
  <div className="rounded-[1.75rem] border border-white/10 bg-stone-950/50 px-5 py-8 text-sm text-stone-300">{label}</div>
);
